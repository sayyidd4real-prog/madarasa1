...
async function calculateStudentBalance(studentId: string): Promise<number> {
  const [rows] = await queryDb(
    "SELECT SUM(amount) as totalCharges, SUM(paid) as totalPayments FROM fees WHERE student_id = ?",
    [studentId]
  );
...
export async function PUT(request: Request) {
  try {
    ...
    const [fRows] = await queryDb("SELECT amount, paid FROM fees WHERE id = ?", [id]);
...
export async function DELETE(request: Request) {
  try {
    ...
    console.error("Finance DELETE API Error:", error);
